...
>  All tutorial actions will take place from inside this folder.
...
>  To stop your containers between steps use `docker kill mongo-toot`. As we are using the `--rm` flag the container will be completely deleted when it exits.

## Persistence

Try inserting a document into your database.
```
docker run -it --rm --name mongo-cli mongo mongo test --eval "db.test.insert({hey: 'there'})" --host 172.17.0.#
```
And now make sure it's there with:
```
docker run -it --rm --name mongo-cli mongo mongo test --eval "db.test.find()" --host 172.17.0.#
```
...
To mount a local file into a container we use the `-v local:remote` argument. For this example we will use `./mongod.conf` provided in the repo. Docker doesn't seem to like it if you use shell expansions for file paths so we shall use command substitution instead. 
...
2. A tidier way is to use a docker network. When running in a docker network, all containers can talk to all other containers using their hostnames (container hash) and container names. All containers will be given separate IP addresses within that network so all nodes can still run on the same port, meaning we can supply them all with the same config file and have no collisions.
...
>  Before you start, make sure to delete any persistent data folders you have used, i.e `./data1`, to get rid of all traces of previous databases.
...
>  In addition to the helper for starting 3 containers, there is also `./kill.sh` to kill all the containers as necessary.
...
>  Even if the node is in the secondary state, you can always exec into the container and perform a mongodump on the data. So taking backups is always an option, you just need to make sure that the data you are backing up is up to date.
